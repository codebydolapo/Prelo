...
function SidebarOption({ href, id }: { href: string, id: string }) {
    ...
    useEffect(() => {
        ...
        if (!yDoc && !yProvider && href.includes(pathName) && pathName == id) {
    ...
    return (
        <Link href={href} className={`relative p-2 rounded-lg cursor-pointer md:w-[12rem] xs:w-[95%] h-[3rem] flex items-center justify-start px-2 space-x-2 ${isActive ? "bg-black text-white shadow-md" : "bg-white shadow-md"}`}>
            ...
            {loaderActive && <svg aria-hidden="true" className="w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
